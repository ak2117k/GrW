...
/**
 * Standalone Jest config for the TDA-008 tamper-evident audit-log tests.
 *
 ...
 * test/tda008 while keeping apps/api as rootDir so ts-jest reads
 * apps/api/tsconfig.json.
 *
 * Run from apps/api:
 *   npx jest --config test/tda008/jest.config.js -v
 */
module.exports = {
  rootDir: path.resolve(__dirname, '../..'),
  roots: ['<rootDir>/test/tda008'],
  ...
  moduleNameMapper: {
    '^otplib$': '<rootDir>/test/tda008/otplib.stub.js',
  },
  transform: {
    '^.+\\.(t|j)s$': [
      'ts-jest',
